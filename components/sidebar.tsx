...
const items: Item[] = [
  ...
  { href: "/messages", label: "Alertes", icon: AlertTriangle },
  { href: "/statistics", label: "Notifications", icon: Bell },
  { href: "/security", label: "Sécurité", icon: Shield },
  { href: "/devices", label: "Microservices", icon: Server },
  { href: "/profile", label: "Profil", icon: UserRound },
  { href: "/signin", label: "Connexion", icon: LogIn },
  { href: "/signup", label: "Inscription", icon: UserPlus },
]
    ...
    >
      <div className="flex items-center justify-between gap-2 px-4 py-5">
        <div className="flex items-center gap-2">
          <div className="size-9 rounded-xl bg-white/20 grid place-items-center font-bold">MS</div>
          <span className={`${open ? "block" : "hidden"} text-sm font-semibold`}>MediSuiv</span>
        </div>
        <button
      ...
      <div className="px-3 pb-5 pt-2">
        <div className={`rounded-2xl bg-white/10 p-3`}>
          <p className="text-xs leading-5">{open ? "Suivi médical intelligent." : "Info"}</p>
        </div>
      </div>
    </aside>
  )
